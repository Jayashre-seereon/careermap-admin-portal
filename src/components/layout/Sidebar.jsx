...
const visibleSections = navSections
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed top-0 left-0 z-50 flex h-full flex-col transition-all duration-300 lg:z-40 ${collapsed ? "w-[72px]" : "w-[240px]"
          } ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        style={{ background: "#fff", borderRight: "1px solid #eee" }}
      >
        {/* Logo */}
        <div
          className={`flex h-20 items-center gap-3 border-b px-4 ${collapsed ? "justify-between" : ""
            }`}
        >
          <div
            className={`overflow-hidden transition-all duration-300 ${collapsed ? "h-16 w-[50px]" : "h-12 w-[150px]"
              }`}
          >
            <img
              ...
              {section.items.map((item) => {
                  ...
                  return (
                    <div key={item.name}>
                      ...
                      <button
                        ...
                        className={`flex w-full items-center gap-3 px-4 py-2 text-sm transition ${isActive || isOpen
                          ...
                          {item.children.map((child) => {
                            ...
                            return (
                              <button
                                ...
                                className={`flex w-full items-center gap-2 px-3 py-[7px] text-sm transition ${isChildActive
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2 text-sm transition ${isActive
                        ? "text-[#9a2119]"
                        : "text-gray-600 hover:text-[#9a2119]"
                      }`}
                  >
