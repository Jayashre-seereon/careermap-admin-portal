...
function QuestionForm({ form, initialValues, sections, disabled, onSubmit, onCancel }) {
  ...
  useEffect(() => { form.setFieldsValue(initialValues || { reverse: false, order: 1 }); }, [form, initialValues]);
  ...
  return <Form form={form} layout="vertical" onFinish={onSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
    ...
    <Form.Item name="order" label="Order" rules={[validationRules.required("Order")]}> <InputNumber disabled={disabled} className="w-full" min={1} /> </Form.Item>
    <div className="md:col-span-2 flex justify-end gap-2"><Button onClick={onCancel}>{disabled ? "Close" : "Cancel"}</Button>{!disabled && <Button htmlType="submit" type="primary">{initialValues ? "Update Question" : "Create Question"}</Button>}</div>
  </Form>;
}
...
export default function Question() {
  ...
  const openForm = (nextMode, item = null) => { setMode(nextMode); setRecord(item); form.resetFields(); if (item) form.setFieldsValue(item); else form.setFieldsValue({ reverse: false, order: 1, sectionId }); setOpen(true); };
  ...
  const columns = [{ title: "#", render: (_, __, index) => index + 1, width: 60 }, { title: "Question", dataIndex: "text", ellipsis: true }, { title: "Response Type", dataIndex: "type", width: 130 }, { title: "Facet", dataIndex: "facet", width: 100 }, { title: "Reverse", dataIndex: "reverse", width: 90, render: (value) => value ? "Yes" : "No" }, { title: "Order", dataIndex: "order", width: 80 }, { title: "Actions", width: 150, render: (_, item) => <div className="flex gap-2"><Button icon={<EyeOutlined />} onClick={() => openForm("view", item)} /><Button icon={<EditOutlined />} onClick={() => openForm("edit", item)} /><Popconfirm title="Delete question?" onConfirm={() => remove(item)}><Button danger icon={<DeleteOutlined />} /></Popconfirm></div> }];
  return <div className="space-y-5"><h2 className="text-xl font-bold text-[#9a2119]">Psychometric Question Management</h2><div className="rounded-2xl border bg-white p-6 shadow-sm"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold text-[#9a2119]">Questions</h2><div className="flex gap-3"><Button icon={<ReloadOutlined />} onClick={() => load(sectionId)}>Reset</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => openForm("add")}>Add Question</Button></div></div><div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2"><Select value={sectionId} placeholder="Select section to show questions" options={sections.map((item) => ({ value: item.id, label: item.title || item.code }))} onChange={(value) => { setSectionId(value); load(value); }} /><Input value={search} onChange={(event) => setSearch(event.target.value)} prefix={<SearchOutlined />} placeholder="Search questions" /></div><Table rowKey="id" columns={columns} dataSource={data} loading={loading} scroll={{ x: "max-content" }} /></div><Modal open={open} onCancel={close} footer={null} width={850} destroyOnClose title={mode === "add" ? "Add Question" : mode === "edit" ? "Edit Question" : "View Question"}><QuestionForm form={form} initialValues={record} sections={sections} disabled={mode === "view"} onSubmit={save} onCancel={close} /></Modal></div>;
}
