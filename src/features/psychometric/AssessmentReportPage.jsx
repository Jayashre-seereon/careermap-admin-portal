...
import { getAttemptDetails, getAttemptResult } from "../../api/psychometricAssessmentApi";
...
import ReportImg1 from "../../assets/report/report_1.png";
import ReportImg2 from "../../assets/report/report_2.png";
import ReportImg3 from "../../assets/report/report_3.png";
import ReportImg4 from "../../assets/report/report_4.png";
import ReportImg5 from "../../assets/report/report_5.png";
import ReportImg6 from "../../assets/report/report_6.png";
import ReportImg7 from "../../assets/report/report_7.png";
import ReportImg8 from "../../assets/report/report_8.png";
import ReportImg9 from "../../assets/report/report_9.png";
import FeaturePsychometric from "../../assets/report/feature_psychometric.jpg";
import FeatureOnetoone from "../../assets/report/feature_onetoone.jpg";
import FeatureMentorship from "../../assets/report/feature_mentorship.jpg";
import FeatureCell from "../../assets/report/feature_cell.jpg";
import FeatureBehavioral from "../../assets/report/feature_behavioral.jpg";
import FeatureDashboard from "../../assets/report/feature_dashboard.jpg";
...
export default function AssessmentReportPage() {
  ...
  async function loadReport() {
    ...
    try {
      if (attemptId && attemptId !== "demo") {
        let res = null;
        try {
          res = await getAttemptDetails(attemptId);
        } catch {
          res = await getAttemptResult(attemptId);
        }

        const data = res?.data || res?.attempt || res;
        if (data && (data.report || data.domainScores || data.scores || data.topClusterName || data.studentName)) {
          setReportData(data);
        } else {
          setReportData(null);
        }
  ...
  const scores = rawData.scores || rawData.domainScores || {};
  const domains = report.domains || rawData.domainScores || {};

  // Goal Orientation
  const goalObj = domains.goalOrientation || domains.goal_orientation || {};
  const longPct = goalObj.longTerm?.percentage ?? goalObj.L ?? (scores.goalLong != null ? pct(scores.goalLong) : 80);
  const shortPct = goalObj.shortTerm?.percentage ?? goalObj.S ?? (scores.goalShort != null ? pct(scores.goalShort) : 100);

  // Interests Scores (RIASEC)
  const riasecObj = domains.riasec || scores.riasec || {};
  const interestScoreMap = {
    E: riasecObj.E != null ? pct(riasecObj.E) : 100,
    C: riasecObj.C != null ? pct(riasecObj.C) : 95,
    S: riasecObj.S != null ? pct(riasecObj.S) : 85,
    R: riasecObj.R != null ? pct(riasecObj.R) : 80,
    I: riasecObj.I != null ? pct(riasecObj.I) : 55,
    A: riasecObj.A != null ? pct(riasecObj.A) : 55,
  };
  const domainInterests = Array.isArray(domains.interests) ? domains.interests : [];
  ...
  const oceanObj = domains.ocean || scores.ocean || {};
  const personScoreMap = {
    ES: oceanObj.ES != null ? pct(oceanObj.ES) : 75,
    O: oceanObj.O != null ? pct(oceanObj.O) : 63,
    Cn: oceanObj.Cn != null ? pct(oceanObj.Cn) : (oceanObj.C != null ? pct(oceanObj.C) : 50),
    Ex: oceanObj.Ex != null ? pct(oceanObj.Ex) : (oceanObj.E != null ? pct(oceanObj.E) : 50),
    Ag: oceanObj.Ag != null ? pct(oceanObj.Ag) : (oceanObj.A != null ? pct(oceanObj.A) : 46),
  };
  const domainPerson = Array.isArray(domains.personality) ? domains.personality : [];
  ...
  const valuesObj = domains.values || scores.values || {};
  const valScoreMap = {
    OC: valuesObj.OC != null ? pct(valuesObj.OC) : 100,
    SE: valuesObj.SE != null ? pct(valuesObj.SE) : 95,
    ST: valuesObj.ST != null ? pct(valuesObj.ST) : 60,
    CO: valuesObj.CO != null ? pct(valuesObj.CO) : 56,
  };
  const domainValues = Array.isArray(domains.values) ? domains.values : [];
  ...
  const varkObj = domains.vark || scores.vark || {};
  const varkScoreMap = {
    V: varkObj.V != null ? pct(varkObj.V) : 100,
    Rd: varkObj.Rd != null ? pct(varkObj.Rd) : (varkObj.R != null ? pct(varkObj.R) : 85),
    A: varkObj.A != null ? pct(varkObj.A) : 75,
    K: varkObj.K != null ? pct(varkObj.K) : 60,
  };
  const domainVark = Array.isArray(domains.learningStyles) ? domains.learningStyles : [];
  ...
  const aptObj = domains.aptitude || scores.aptitude || {};
  const aptScoreMap = {
    Verb: aptObj.Verb != null ? pct(aptObj.Verb) : 43,
    Log: aptObj.Log != null ? pct(aptObj.Log) : 33,
    Voc: aptObj.Voc != null ? pct(aptObj.Voc) : 33,
    Mech: aptObj.Mech != null ? pct(aptObj.Mech) : 33,
    Spat: aptObj.Spat != null ? pct(aptObj.Spat) : 15,
    Num: aptObj.Num != null ? pct(aptObj.Num) : 14,
  };
  const domainApt = Array.isArray(domains.aptitudes) ? domains.aptitudes : [];
  domainApt.forEach((d) => {
    if (d.facet && (d.percentage != null || d.score != null)) {
  ...
  const defaultTop5 = [
    ...
    },
    {
      ...
      streams_and_pathways_india: "Any stream. Pathways: CLAT/AILET for law, UPSC civil services examinations after graduation in any stream.",
      careers: [
  ...
  const rawTop5 = report.careerClusters?.top5 || rawData.top5Clusters || rawData.clusterRecommendations || [];
  ...
  const top5Clusters = rawTop5.length >= 5
    ? rawTop5.slice(0, 5).map((item, idx) => {
        const code = item.code || item.cluster_id || item.clusterId || defaultTop5[idx].code;
        const meta = clusterMap[code] || clusterMap[item.name || item.cluster || item.clusterName] || CLUSTERS[idx % CLUSTERS.length] || {};
        return {
          rank: idx + 1,
          code,
          name: (item.name || item.cluster || item.clusterName || meta.name || defaultTop5[idx].name).toUpperCase(),
          matchPercentage: item.matchPercentage ?? item.matchPercent ?? item.match ?? defaultTop5[idx].matchPercentage,
    ...
    <div className="report-app-container">
        ...
        <div className="report-action-bar-inner">
          <div className="flex items-center gap-3">
            <Button
              ...
              onClick={() => navigate("/admin/psychometric-attempts")}
      ...
      <div className="pdf-pages-wrapper">
        ...
        <div className="pdf-page relative overflow-hidden flex flex-col justify-between" id="page-1">
          ...
          {/* Center Graphic: Large Hero 3D Brain Illustration */}
              ...
              <div>
                <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path stroke="#466CA3" strokeDasharray={`${varkScoreMap.V}, 100`} strokeWidth="5" strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
                  <div className="absolute font-black text-2xl text-[#1E3A8A]">{varkScoreMap.V}%</div>
                </div>
              ...
              <div>
                <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path stroke="#4D6D47" strokeDasharray={`${varkScoreMap.A}, 100`} strokeWidth="5" strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
                  <div className="absolute font-black text-2xl text-[#154512]">{varkScoreMap.A}%</div>
                </div>
              ...
              <div>
                <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path stroke="#B58E2E" strokeDasharray={`${varkScoreMap.Rd}, 100`} strokeWidth="5" strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
                  <div className="absolute font-black text-2xl text-[#5C450A]">{varkScoreMap.Rd}%</div>
                </div>
              ...
              <div>
                <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path stroke="#9A4235" strokeDasharray={`${varkScoreMap.K}, 100`} strokeWidth="5" strokeLinecap="round" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
                  <div className="absolute font-black text-2xl text-[#691811]">{varkScoreMap.K}%</div>
                </div>
          ...
          <div className="pdf-page-body space-y-3.5">
            ...
            <div className="flex justify-center gap-16 text-center py-1">
              <div>
                ...
                <div className="mt-1 text-xs font-bold text-slate-700">SHORT TERM</div>
              </div>
              <div>
                <div className="relative w-28 h-28 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                ...
                <div className="mt-1 text-xs font-bold text-slate-700">LONG TERM</div>
              </div>
            </div>

            <div className="p-3 bg-[#94721C] text-white rounded-xl text-center font-extrabold text-sm uppercase tracking-wider shadow-xs">
              Most Inclined towards : {shortPct >= longPct ? "SHORT TERM" : "LONG TERM"}
            </div>
          </div>
